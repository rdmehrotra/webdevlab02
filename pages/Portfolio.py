...
def links_section():
    ...
    st.sidebar.text("I'm even cooler on Instagram.")
    linkedin_link = f'<a href="{info.my_instagram_url}"><img src="{info.instagram_image_url}" alt="Instagram" width = "75" height = "75"></a>'
    st.sidebar.markdown(linkedin_link, unsafe_allow_html=True)
    st.sidebar.text("Or Snap if you don't have Instagram.")
    github_link = f'<a href="{info.my_snapchat_url}"><img src ="{info.snapchat_image_url}" alt = "Snapchat" width="75" height="75"></a>'
    st.sidebar.markdown(github_link, unsafe_allow_html=True)
    st.sidebar.text("Or if you really hate socials, email me I guess.")
...
def activities_section(leadership_data, activity_data):
    ...
    with tab2:
        st.subheader("️️🏘️ Community Service")
        for title, (bullet_points, image) in activity_data.items():
            expander = st.expander(f"{title}")
            expander.image(image, width=250)
